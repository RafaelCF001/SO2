...
public class MyKernel implements Kernel {
    ...
    public String cd(String parameters) {
        ...
        for(String path: parameters.split("/")){
            ...
            if (path.equals("..")){
                currentDir = struct.get(diretorioAtual).fileParent.nome;
    ...
    public String mv(String parameters) {
        ...
        FileStruct fileTo = new FileStruct("cmp");
        
		for(String path: parameters.split("/")) {
				if(path.equals("..")) {
					fileTo = struct.get(path).fileParent;
				}
				else if(path.equals(".")) {
					fileTo = struct.get(path);
				}
				else {
					fileTo = struct.get(path);
				}
		...
		FileStruct.copyChild(struct.get(diretorioAtual),fileTo); 
